...
class Backtest:
    def __init__(self, exchange: Exchange, period_start: datetime, period_end=None, interval=60):
        ...
        pair = exchange.strategy.pair
        # Try to find dataset
        try:
            dataset = Dataset().get({"exchange": exchange.name.lower(),
                                     "currency": pair.currency.lower(),
                                     "asset": pair.asset.lower(),
        ...
        if dataset and len(dataset) > 0:
            ...
            for price in exchange.historical_symbol_ticker_candle(pair, period_start, period_end, interval):
